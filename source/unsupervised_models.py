...
def train_one_class_svm(X_train, X_test):
    ...
    y_true = [1] * len(y_pred_test)  # Assuming normal data for unsupervised testing
...
def train_isolation_forest(X_train, X_test):
    ...
    # Prediction time
    prediction_time = time.time() - start_time
    print(f"Prediction using Isolation Forest took {prediction_time:.2f} seconds")
...
def train_elliptic_envelope(X_train, X_test):
    ...
    elliptic_env = EllipticEnvelope(contamination=0.1, support_fraction=0.9, random_state=42)
    ...
    # Prediction time
    prediction_time = time.time() - start_time
    print(f"Prediction using Elliptic Envelope took {prediction_time:.2f} seconds")
